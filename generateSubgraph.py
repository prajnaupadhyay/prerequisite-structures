# Does the following
# 1) Returns the Wikipedia subgraph containing the links upto two hops for a list of input concepts and stores it in file.
...
class ABContentHandler(xml.sax.ContentHandler):
	def __init__(self):
		...
		for i in range(len(self.inputConcepts)):
			output = "/home/prajna/neo4j/input/toposortTrees/subsetOfTreesAllLinks/nodes/"+self.inputConcepts[i]
			output1 = "/home/prajna/neo4j/input/toposortTrees/subsetOfTreesAllLinks/"+self.inputConcepts[i]
			self.outputFiles[i] = open(output,"w")
			self.outputFiles1[i] = open(output1,"w")
				...
				while(len(self.queue)<>0):
					...
					for item in self.dict1[a[0]]:
						...
						if(item not in self.queue2 and item not in self.visited):
							...
							self.dict2[item]=a[0]
				self.queue1.sort(key=operator.itemgetter(1))
				
				onlyItems = []
				
				for item in self.queue1:
					onlyItems.append(item[0])
				
				print len(onlyItems)
				
				for item in onlyItems:
					self.outputFiles[counter].write(str(item)+"\n")
					
					neighbours = set(onlyItems).intersection(self.dict1[item])
					for elem in neighbours:
						self.outputFiles1[counter].write(str(item)+"\t"+str(elem)+"\n")
